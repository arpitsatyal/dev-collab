...
import {
  ...
  semanticSearchTool,
} from '../../../../../dev-collab-client/src/lib/ai/services/toolService';
import {
  performHybridSearch,
  generateQueryVariations,
} from '../../../../../dev-collab-client/src/lib/ai/services/retrievalService';
import {
  constructPrompt,
  buildChatMessages,
  ...
  IntentSchema,
} from '../../../../../dev-collab-client/src/lib/ai/services/promptService';
...
async function getAIResponseWithTools(
  chatId: string,
  question: string,
  workspaceId: string,
) {
  ...
  const messages: BaseMessage[] = buildChatMessages(history, question);
  let iterations = 0;
  ...
  while (iterations < MAX_CHAT_ITERATIONS) {
    ...
    console.log(
      `[Chat Engine] Iteration ${iterations}: executing ${response.tool_calls.length} tool(s).`,
    );
...
export async function getAIResponse(
  ...
  const classifierLlm = await getReasoningStructuredLLM(
    IntentSchema,
    'classify_intent',
  );
  ...
  if (intent === 'CONVERSATIONAL') {
    if (scope === 'OUT_OF_SCOPE') {
      return {
        answer: APP_SCOPE_REPLY,
        context: '',
        validated: { isValid: true, warning: null },
      };
    }
    const history = await getChatHistory(chatId);
    const conversationalMessages = buildConversationalMessages(
      history,
      question,
    );
