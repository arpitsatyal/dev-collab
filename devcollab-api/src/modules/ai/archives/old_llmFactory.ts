...
export function getLLM(provider?: LLMProvider): BaseChatModel {
  const selectedProvider =
    provider || (process.env.LLM_PROVIDER as LLMProvider) || 'TOGETHER';
  ...
  switch (selectedProvider) {
    ...
    default:
      console.warn(
        `Unknown LLM provider: ${selectedProvider}. Defaulting to TOGETHER.`,
      );
      return new TogetherLLM({});
  }
}
...
class FallbackLLM extends SimpleChatModel {
  ...
  async _call(messages: BaseMessage[]): Promise<string> {
    try {
      ...
      if (isRateLimit) {
        console.warn(
          `[LLM] ${this.primaryName} failed (rate limit/unavailable), falling back to ${this.fallbackName}...`,
        );
