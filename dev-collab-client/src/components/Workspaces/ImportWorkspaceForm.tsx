...
import {
  ...
  useImportRepoTree,
} from "../../hooks/mutations/useImportWorkspaceMutations";
import {
  Box,
  Divider,
  Group,
  ...
  useEffect(() => {
    if (treeError) {
      notifications.show({
        title: "Fetch Failed",
        message:
          (treeError as any).data?.error || "Failed to fetch repo structure",
        color: "red",
      });
      setRepoUrl(null);
    }
        ...
        <BaseCard className={classes.root}>
          <Stack gap="md">
            ...
            <Text size="sm" c="dimmed">
              Enter a public GitHub repository URL to browse its files and
              select context to import.
            </Text>
        ...
        <Stack gap="md">
          <Group justify="space-between">
            ...
            <BaseBadge
              size="lg"
              variant="light"
              color={selectedFiles.length === MAX_FILES ? "orange" : "blue"}
            >
